...
gsigs = 'https://github.com/amonbit-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('amonbit'):
        # Remove the potentially stale amonbit dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'amonbit', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'amonbit'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/amonbit/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'amonbit='+args.commit, '--url', 'amonbit='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/amonbit-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/amonbit/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/amonbit/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/amonbit-project/amonbit', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/amonbit', exist_ok=True)
    os.chdir('builder/inputs/amonbit')
    if args.pull:
