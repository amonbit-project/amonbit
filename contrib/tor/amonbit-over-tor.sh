...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/amonbitd"
  then
    amonbitd="$dir/amonbitd"
    break
  fi
done
if test -z "$amonbitd"
then
  echo "amonbitd not found"
  exit 1
fi
echo "Found: $amonbitd"

TORDIR="$DIR/amonbit-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting amonbitd"
  tail -n 400 "$HOME/.bitamonbit/bitamonbit.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Monero\ \'" | tail -n 20
  exit 1
fi
